...
payables_df =  df_payables[df_payables['reconcile_flag'] == 0]
payables = round(np.sum(payables_df['amount']),2)
receivables_df =  df[(df['pmt'] == 'income') & (df['reconcile_flag'] == 0)]
...
grp_income = df_income.groupby(['account','pmt'])
grp_expense = df_expense.groupby(['account','pmt'])
...
if (recon_number < 0.01) & (recon_number > -0.01):
    print("***Checking account balance matches ledger balance (RECONCILED)")
elif recon_number > 0:
    print("!!!Checking balance HIGHER than accounted by " + recon_number_f)
else:
    print("!!!Checking balance LOWER than accounted by " + recon_number_f)
...
if (recon_number < 0.01) & (recon_number > -0.01):
    print("***Checking account balance matches ledger balance (RECONCILED)", file=f)
elif recon_number > 0:
    print("!!!Checking balance HIGHER than accounted by " + recon_number_f, file=f)
else:
    print("!!!Checking balance LOWER than accounted by " + recon_number_f, file=f)
